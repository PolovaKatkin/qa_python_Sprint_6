...
class MainPage(BasePage):
    @allure.title('Открываем главуню страницу')
    def open_main_page(self):
        self.open_page(Urls.MAIN_PAGE_URL)

    @allure.title('Прокручиваем вниз страницу до блока "Вопросы о важном", ожидаем загрузку блока')
    ...
    @allure.title('Ожидаем, пока прогрузится форма заказа')
    def wait_for_load_form(self):
        self.wait_for_load_element(OrderPageLocators.FORM1_TITLE)

    @allure.title('Вспомогательная функция для получения текста вопроса в блоке "Вопросы о важном"')
    ...
    @allure.title('Вспомогательная функция для получения текста ответа в блоке "Вопросы о важном"')
    ...
    @allure.title('Получаем ожидаемый URL страницы заказа')
